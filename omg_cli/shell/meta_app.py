...
class MetaApp(App):
    ...
    def _register_import_command(self) -> None:
        ...
        async def import_handler(ctx, args: str) -> None:
            await self.start_import_wizard()

        self.context.register_command(
            MetaCommand(
                name="import",
                description="Import a model configuration",
                description_zh="导入模型配置",
    ...
    async def on_composer_text_area_submitted(self, event: ComposerTextArea.Submitted) -> None:
        ...
        if self._is_processing:
            ...
            await pending_display.update_messages(self.context.pending_messages)
            return
    ...
    async def _submit_text(self, text: str) -> None:
        ...
        try:
            ...
            await pending_display.update_messages([])
            self._sync_composer_height()
            composer.focus()
